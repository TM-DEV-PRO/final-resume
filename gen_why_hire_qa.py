#!/usr/bin/env python3
"""Generate the 'Why hire Tarun' + screening Q&A pack for every resume track.

Each track gets stack-correct wording (languages on the summary line, FRM stack,
AssortSmart write-plane stack, PDF name, contact) while the facts, honesty tags,
and numbers stay identical across tracks.
"""
...
# dir, label, languages, frm stack, IA write plane, pdf, email, phone, stack (prose bio)
TRACKS = [
    dict(
...
**Track:** {label} · **PDF:** `{pdf}` · **Contact:** {email} · {phone}
...
> cloud-native, high-throughput distributed systems using **{stack}**. He has a proven track record
...
cloud-native distributed systems in {langs}. I've owned finance-grade services at Uber, scaled a
...
distributed systems in {langs}. At Uber, via EPAM, I owned the FRM Risk Scoping backend —
...
def main() -> None:
    written = []
    for t in TRACKS:
        d = os.path.join(BASE, t["d"])
        if not os.path.isdir(d):
            print("skip (no dir):", t["d"])
            continue
        out = os.path.join(d, "38_why_hire_tarun_qa.md")
        with open(out, "w", encoding="utf-8") as fh:
            fh.write(TEMPLATE.format(**t))
        written.append(os.path.relpath(out, BASE))
    for w in written:
        print("wrote", w)
    print(f"{len(written)} Q&A packs")
