...
- **A:** Inverted the workflow: the user states intent (hierarchy + reference period — the only non-derivable inputs); the agent selects features by statistical significance, optimizes cluster count via elbow + silhouette scoring, explores the configuration space internally, and presents **3–5 distinct ranked scenarios** (HLR cap; a baseline scenario mimicking the client's legacy configuration always included) with plain-English rationale — the human approves at explicit gates. Kept the manual wizard alive as a legacy path per client so adoption is opt-in, not forced.
- **R:** Committed targets: <1 h to a finalized plan (from days), 3–5 ranked scenarios presented (from 1 config ever tried), <2% failures, agent-selected features and k fully visible to the planner — with write-back into existing product tables unchanged.
