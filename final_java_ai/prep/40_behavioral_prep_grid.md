...
|---|---|---|---|---|---|
| Ownership / end-to-end | Arch owner; writes=Spring Boot; agent=Python FastAPI + LangGraph; CH POC from 189s to 12.3s MEASURED | 30+ APIs, 8 screens, $340M, led 3 | Selenium→Kafka→Flink→RAG gate; 24h→2h; $600K+/yr | Spring Boot strangler; 1M+/day; mentored 2 | Django reliability |
...
Keep/Drop + dig-deeper QnA from assort_kd_flow; agents SELECT-only; 300-gold / ≥80% offline = promotion gate (not multi-tenant GA). Write APIs Spring Boot; agent plane Python, FastAPI, LangGraph, MCP; ClickHouse POC from 189s to 12.3s (~15.5×) MEASURED. **Verbal only / not on PDF:** Cluster Recommendation Copilot · Hindsight (building).
