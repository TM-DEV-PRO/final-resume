...
> **RESUME ALIGNMENT (Jul 2026):** Menu PDF is **Selenium + Kafka + Flink + LangChain RAG / Gemini / **Milvus** converting multilingual PDFs/images into Uber Eats schema (98%/100% offline; no SFT). **ANZ is a separate Uber Mobility project** — Uber drivers in ANZ, driver+vehicle docs vs local authorities — **not** Eats; 20h/week is HISTORICAL past-resume only. Masters also owns Kafka for GST IRP (different product). Use [`23b_uber_interview_packs.md`](23b_uber_interview_packs.md) § Menu / ANZ as the canonical pack.
