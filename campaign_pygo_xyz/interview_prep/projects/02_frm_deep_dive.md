...
**Bullet 1: Built Uber's FRM Risk Scoping platform (FastAPI, MySQL, React), 8 screens and 30+ REST APIs, automating quarterly risk scoping at $340M group materiality, targeting a 70% cut in manual reconciliation.**
- 8 screens: SOLID (TDD + `Scoping Features.docx`).
- 30+ REST APIs: SOLID (32 `add_api_route` registrations, 31 functional plus health). Fallback: "30-plus endpoints; the exact code count is 31 functional plus a health route, I say 30+ to stay conservative."
- $340M group materiality: SOLID but period-specific. Fallback: "that was the Q4 2025 close figure, stored in `frm_metrics_table`, not a fixed constant."
- 70% cut: NEEDS CARE, it is a TDD target, not a measured result. Fallback: "targeting a 70% reduction, that is the design goal in TDD section 3.1, I do not claim it was measured post-launch; the estimated baseline was roughly a two-week manual close down to three or four days."
- Overall verdict: SOLID with the 70% qualified as a target.

**Bullet 2: Modeled recursive financial hierarchies in an 11-table SQLAlchemy 2.0 MySQL schema and built the rule-based engine that auto-flags material items across 55 line items and 14 entities.**
- 11-table SQLAlchemy 2.0 schema: SOLID (11 ORM models in `database/models/`). Fallback: "11 SQLAlchemy models in the scoping service; the full physical schema has 16-plus tables including the separate collaboration service, so I scope the claim to the scoping service's models."
- Recursive hierarchies: SOLID (flattened into `level_mapping` `level1..level4` with stable `level_id`).
- Rule engine: SOLID (`conclusion_overall_significance_from_balance_sheet_materials`, quantitative-OR-qualitative, plus the 5% component rule).
- 55 line items: SOLID but explain the split. Fallback: "about 26 balance-sheet plus 29 income-statement FSLIs on the Q4 2025 close, which is where 55 comes from."
- 14 entities: SOLID for the Q4 2025 sample. Fallback: "14 entities in that close's scoping set; the broader GL has around 400 entities, so I am referring to the scoped set."
- Overall verdict: SOLID.

**Bullet 3: Migrated Sheets-backed reconciliation to MySQL v2 APIs, validating HFM extracts against public 10-Q filings and fixing a latent column-aliasing bug.**
- Migration ownership: SOLID and personally owned (branch `tmitta1/recon-income-api-migration`, 18 files, +1,268 lines, `RECON_API_MIGRATION.md`).
- HFM vs 10-Q validation: SOLID (v2 returns per-line `difference = hfm_amount_in_millions - financial_statement_amount_in_millions`). Fallback: "the tool surfaces and persists the tie-out difference per line; final sign-off on breaks is procedural, done by managers."
- Column-aliasing bug: SOLID as a code pattern (`dict(zip(result.keys(), row))` collision, fixed with `aliased()` and `.label()`). Fallback if pressed on whether it was a live production incident: "the ambiguous-keys pattern was real in the legacy read path and I fixed it during the ORM move; I present it as a latent bug I removed, not necessarily a customer-facing outage."
- Overall verdict: SOLID.

**Bullet 4: Led 3 engineers, owning design reviews, API contracts, and CI quality gates for releases feeding PwC audit work papers.**
- Led 3 engineers: NEEDS CARE on title, SOLID on headcount (user-confirmed EPAM pod). Fallback: "tech lead of a 3-engineer EPAM pod, not a people manager."
- Design reviews and API contracts: SOLID (enforced layered conventions, Pydantic schemas, backend team syncs).
- CI quality gates: SOLID (Bazel `uber_py_test`, tests required per PR, roughly 1,288 test functions, resume says 1,100+).
- Feeds PwC audit work papers: SOLID (TDD states scoping output feeds the external auditor).
- Overall verdict: SOLID with the "led" framed as pod tech lead.
