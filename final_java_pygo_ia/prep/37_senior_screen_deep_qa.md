...
**Stack note (this hybrid PDF):** FRM + Masters = **Spring Boot**. AssortSmart write APIs = **Go Gin** (same as PyGo IA). Chat plane = FastAPI / LangGraph / MCP. Menu/GFG stay Python-path truthful.
...
**Answer:** AssortSmart is a **merchandise planning SaaS** (**AssortSmart (Retail Merchandise Planning Platform)** on this PDF). Retail planners decide **what to buy, how much, and which stores** for a season ahead. On the resume I architected AssortSmart's **Keep/Drop engine** (ST%/ROS + LangGraph lenses; agents SELECT-only on ClickHouse via CSV bake-and-promote; promotions gated on 300 gold / ≥80% offline) and a **read-only dig-deeper QnA** over locked decisions. Write plane: **Go / Gin**. I also drove **ClickHouse** adoption after a row-identical Postgres-versus-ClickHouse POC (**250M** rows, from **189s to 12.3s**, ~**15.5×** MEASURED). **Verbal only / not on PDF:** Cluster Recommendation Copilot · Hindsight.
...
**Answer:** Dual deployables — **Python agent** and **Go Gin** write services + per-tenant ClickHouse. (FRM/Masters remain Spring Boot.) Ship independently. Hindsight metric catalogs can go live **without a code deploy**. Observability: Datadog + LangSmith + PostHog on one `trace_id`. **Honesty:** load test pending → say building.
...
Java/Spring for FRM/Masters; Python+Go for AssortSmart (FastAPI/LangGraph/MCP + Gin); Python for RAG/Menu scrapers — same products/metrics.
