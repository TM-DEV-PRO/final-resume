...
**Purpose:** Anything on `Tarun_Mittal_SSE_5yr.pdf` you can defend in depth.
Canonical: `docs/ASSORTSMART_TAB_RESUME.md`.

**PDF IA (Sep 2026):** Platform (10k RPS, CH 189s to 12s / six tables / 170 GB OOM, KPI parser, OIDC, 100%/1200+ tests) + Agentic (348k article-seasons, 88k/pass, 7 lenses, 2.11B JSON payloads, Ask Iris shipped, 300-case / 80% vs 74%).
...
### Architecture
```
Finance UI → FastAPI / SQLAlchemy 2.0 (36 FastAPI endpoints)
...
| Tech | Number |
|---|---|
| FastAPI / SQLAlchemy 2.0 | 36 FastAPI endpoints |
...
FastAPI strangler: p95 1.2s→300ms (75%), 700→4000 rpm, Kafka+PG tax-quarter shard, 1M+/day, 35%→82%, 98% deploy.
