...
1. Owned FRM scoping backend, **70%** recon cut (**14 days to 3 days**) against **$340M** materiality. Sheets→MySQL across **19M** raw GL rows. **36 FastAPI endpoints**. Nested **L1–L4 FSLI** with quarter-annualization.
...
3. Led **3** engineers on a SQLAlchemy 2.0 migration to **100%** statement coverage. Stacked Bazel PRs, SQL allowlists, FSM **50%** delta-variance gates for SOX.
...
**Bullet 1: Owned Uber's FRM Risk Scoping backend (FastAPI, MySQL), 30+ REST APIs powering 8 screens, automating quarterly close at $340M group materiality, targeting a 70% cut in manual reconciliation (~2 weeks → ~3–4 days).**
