...
Aligned to **Final PyGo + AI** resume (`Tarun_Mittal_SSE_PyGo_AI_Final.pdf`). Paste-ready. Avoid colon-heavy AI voice in About.

## Headline (pick one)
1. Senior Software Engineer | Python · Go | Distributed Systems · Kafka · ClickHouse · LangGraph
2. Backend SSE (Python and Go) | Uber (via EPAM) FRM · Kafka 1M+/day · Go/Gin 10k RPS · LangGraph
...
**FRM:** Owned FRM scoping backend (FastAPI / SQLAlchemy 2.0) — 36 FastAPI endpoints, 19M raw GL rows, L1–L4 FSLI, 8-table MySQL SOADB, SHA-256 natural keys, optimistic locking. 70% recon cut from 14 days to 3 against $340M. Led 3 on the SQLAlchemy 2.0 migration to 100% statement coverage; Bazel PRs; SQL allowlists; FSM 50% delta-variance for SOX.
...
Cut p95 from 1.2s to 300ms (75%) for 1,500+ clients by migrating a PHP monolith to FastAPI microservices; mentored 2. 700 to 4,000 rpm and 1M+ IRP/day on Kafka + PostgreSQL sharded by tax quarter. Idempotency, DLQ, Redis −30% reads. Coverage 35% to 82% at 98% deploy.
...
## Featured section
- Resume PDF (Python/Go FRM + AssortSmart platform/AI)
- LangChain Academy cert
- HackerRank Problem Solving cert
- Optional: GitHub TM-DEV-PRO

## Skills order (LinkedIn)
Python · Go · Gin · FastAPI · Distributed Systems · Microservices · Kafka · ClickHouse · System Design · Docker · AWS · GCP · PostgreSQL · MySQL · Redis · LangGraph · RAG · Mentorship\n