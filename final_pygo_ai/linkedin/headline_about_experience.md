...
Aligned to **Final PyGo + AI** resume. Paste-ready. Avoid colon-heavy AI voice in About where possible.

## Headline (pick one)
1. Senior Software Engineer | Python · Go | Distributed Systems · Kafka · AI-assisted systems
2. Backend SSE (Python and Go) | Owned Uber (via EPAM) FRM design · Kafka 1M+/day · LangGraph + ClickHouse
...
Cut p95 from 1.2s to 300ms for 1500+ clients by migrating Laravel to FastAPI microservices; mentored 2. Lifted from 700 to 4000 req/min and 1M+ IRP/day on Kafka + PostgreSQL split by tax quarter. Idempotency keys, retries, DLQ + Redis −30% reads. ELK/New Relic + usage dashboard cut triage 70% and support tickets 35%; coverage from 35 to 82% at 98% deploy.
