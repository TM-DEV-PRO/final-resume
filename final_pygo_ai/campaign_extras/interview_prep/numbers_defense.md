...
| **ONE CH bullet (PDF):** Drove adoption of **ClickHouse** as AssortSmart's planning analytics engine, improving pivot latency **189s → 12.3s** (~**15.5×**) on **250M** rows by running a row-identical **Postgres → ClickHouse** POC. | Planning analytics adoption + pivot POC | MEASURED | `pivot-poc` / `21_…` |
...
| 11-table schema | **Verbal schema depth** (supporting); PDF leads with materiality engine + recon v2 | MEASURED | deep dive 11 |
| 18-file recon migration | On PDF | MEASURED | deep dive 11 |
