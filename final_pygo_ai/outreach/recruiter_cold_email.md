...
Aligned to **Final Python + Go + AI** (`Tarun_Mittal_SSE_5yr.pdf`). Paste-ready. Adapted from the AssortSmart-tab packet templates; claims match the **current PDF**, not packet overclaims (no K8s-ops, no IA Flink/CDC).
...
## LinkedIn DM long
Hi [Name] — hire-for-fit in one glance: Uber (via EPAM) FRM owner (36 FastAPI endpoints, 70% 14d to 3d, led 3) + Menu E2E ($600K/yr, 24h to 2h, Kafka/Flink) + Masters Kafka 1M+/day + current AssortSmart Go/Gin 10k RPS, ClickHouse six rollup tables, Go pump at about 344k rows/s combined on a 4.09B-row attr weekly set, and shipped Ask Iris. Looking at [Company] senior backend / data platform / applied-AI roles. If you are sourcing for [Team] I would love a quick screen. Resume here: [link].
