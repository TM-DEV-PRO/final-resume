...
**Resume tech:** Java, Spring Boot, Spring WebFlux, Hibernate, PostgreSQL, Redis, Spring Batch, Docker, ELK, New Relic, AWS

---

## 1. Elevator pitch

"Masters India is a GST Suvidha Provider — enterprises file GST returns and generate e-invoices through our APIs. I led the migration of core compliance APIs from a PHP (Laravel) monolith to Java Spring Boot / WebFlux microservices: p95 latency 1.2s → 300ms, scaled to 1M+ daily transactions for 2,500+ enterprise clients. Bulk e-invoicing — 100K+ transactions per import — was a chief revenue path."
...
3. **WebFlux + WebClient** for outbound IRP calls — non-blocking wait; concurrency without one OS thread per in-flight government call.
...
- **"WebFlux vs MVC?"** Outbound-I/O-bound government calls → reactive client wins. Prefer MVC + virtual threads if the team isn't fluent in reactive operators — be honest about team skill as a tradeoff.
